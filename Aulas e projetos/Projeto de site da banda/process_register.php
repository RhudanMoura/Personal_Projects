...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // 🔑 Verificação do código do administrador ANTES de inserir no banco
    $codigo_correto = "123456";
    if ($user_type === "admin") {
        if (empty($admin_code)) {
            $response["message"] = "Para se cadastrar como administrador, é necessário informar o código de administrador.";
            $_SESSION['register_response'] = $response;
            header("Location: register.php");
            exit;
        }

        if ($admin_code !== $codigo_correto) {
            $response["message"] = "Código de administrador incorreto (Código para teste -> 123456). Caso você NÃO tenha o código, registre-se como Utilizador.";
            $_SESSION['register_response'] = $response;
            header("Location: register.php");
            exit;
        }
